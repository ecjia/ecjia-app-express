										...
										<!-- {foreach from=$express_user_list.list item=list} -->
											{if $list.online_status eq '4'}
												<div class="express-user-info" online_status="{$list.online_status}">
													...
													<div class="assign-div">
														 <button class="btn" type="button" disabled="disabled" data-toggle="modal" href="#assignmodel" notice="是否确定让  【{$list.name}】  去配送？" assign-url='{url path="express/admin/assign_express_order" args="staff_id={$list.user_id}&type={$type}"}'><span>指派给他</span></button>  
													</div>
												</div>
											{/if}
										<!-- {/foreach} -->
