...
class grab_list_module extends api_admin implements api_interface {
    public function handleRequest(\Royalcms\Component\HttpKernel\Request $request) {	
		...
		$where = array('eo.store_id' => $_SESSION['store_id'], 'staff_id' => 0, 'eo.from' => '', 'eo.status' => 0);
		...
		if (!empty($express_order_result)) {
			foreach ($express_order_result as $val) {
				$express_order_list[] = array(
						...
						'best_time'		=> empty($val['best_time']) ? '' : RC_Time::local_date(ecjia::config('time_format'), $val['best_time']),
